        ...
        <>
            ...
            <Stack spacing="8" width="100%">
                <Stack spacing="5" w="100%" minWidth="300px" justify="space-between" alignItems="left" bg="white" borderRadius="16px" shadow="xl" px="8" py="8">
                    ...
                    <Stack spacing="1">
                        <Text href="/caixa" display="flex" alignItems="center" fontSize="sm" color="gray.600">Pendência de venda</Text>  
                        <Text href="/caixa" display="flex" alignItems="center" fontSize="md" color="gray.700">{Intl.NumberFormat('pt-BR', {style: 'currency', currency: 'BRL' }).format(pendingSales)}</Text> 
                    </Stack>
                </Stack>

                <Stack spacing="5" w="100%" minWidth="300px" justify="space-between" alignItems="left" bg="white" borderRadius="16px" shadow="xl" px="8" py="8">
                    <HStack>
                        <Text fontSize="xl" w="100%">Meta do mês</Text>
                        <Text href="/caixa" display="flex" alignItems="center" fontSize="sm" color="gray.600">{monthGoal ? '' : 'Indefinida'}</Text> 
                    </HStack>

                    <HStack>
                        <Text fontSize="xl" w="100%" fontWeight="semibold" color="gray.800">{Intl.NumberFormat('pt-BR', {style: 'currency', currency: 'BRL' }).format(monthGoal ? monthGoal.value : 0)}</Text>
                    </HStack>

                    <Stack spacing="1">
                        <Text href="/caixa" display="flex" alignItems="center" fontSize="sm" color="gray.600">Visitas</Text>  
                        <Text href="/caixa" display="flex" alignItems="center" fontSize="md" color="gray.700">{monthGoal?.visits}</Text> 
                    </Stack> 

                    <Stack spacing="1">
                        <Text href="/caixa" display="flex" alignItems="center" fontSize="sm" color="gray.600">Fechamentos</Text>  
                        <Text href="/caixa" display="flex" alignItems="center" fontSize="md" color="gray.700">{monthGoal?.sales}</Text> 
                    </Stack> 
                </Stack>
