          ...
          {!data ? (
            ...
            <FormControl pos="relative" isInvalid={!!error} w="fit-content">
              ...
              <Select
                options={leadOptions}
                styles={customStyles}
                onChange={value => handleChangeUser(value ? value.value : '')}
              />
            </FormControl>
          )}
        </Stack>
          ...
          >
            {Object.keys(leadsReport).map((status: string, index: number) => {
              return (
                <Tr>
                  <Th
                    color={`${leadsReport[status]['color']}.500`}
                    whiteSpace="nowrap"
                    fontSize="sm"
                    bg="white"
                    position="sticky"
                  ...
                  {Object.keys(leadsReport[status]).map(
                    (month: string, index: number) => {
                      ...
                      return month !== 'total' ? (
                        <Th
                          whiteSpace="nowrap"
                          fontWeight="50"
                          key={`${status}-${month}-${leadsReport[status][month]['count']}`}
                        >
                          <Stack>
                            <Text color="gray.900" fontWeight="normal">
                              {leadsReport[status][month]['count']} (
                              {leadsReport[status][month]['percent']}%)
                            </Text>
