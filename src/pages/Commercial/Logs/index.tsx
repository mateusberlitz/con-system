import { Divider, Flex, HStack, Link, Spinner, Stack, Text, Th, Tr } from "@chakra-ui/react";
...
export function Logs(){
    ...
    const { handleSubmit, register, formState, control} = useForm<LogsFilterData>({
                ...
                <HStack justifyContent="space-between">
                    <HStack spacing="3" height="50px">
                        ...
                        <Stack>
                            ...
                            <Text fontSize="lg" fontWeight="semibold" color="gray.700">{userData?.name} {userData?.last_name}</Text>
                        </Stack>
                    </HStack>

                    <HStack as="form" spacing="6" onSubmit={handleSubmit(handleChangeFilter)}>
                        <Input register={register} name="search" type="text" placeholder="Ação executada" variant="filled" focusBorderColor="orange.400" error={formState.errors.search}/>
                                        ...
                                        return (
                                            <Tr>
                                                <Th color="gray.800" fontWeight="normal">
                                                    <Text fontSize="10px">{formatBRDate(log.created_at)}</Text>
                                                    <Text fontSize="sm">{getHour(log.created_at)}</Text>
                                                </Th>
                                                <Th color="gray.700" fontWeight="normal" textTransform="capitalize">{log.action}</Th>
                                            </Tr>
                                        )
                                    })
                                }
