...
export function ScheduleRow({hourSchedules, hour, days, handleEditSchedule, handleRemoveSchedule, handleCompleteSchedule}: ScheduleRowProps){
                            ...
                            <Th key={Math.random().toString(36).substr(2)} minWidth="154px" p="1" borderBottom="none" borderLeft="1px solid #e2e8f0" borderRight="1px solid #e2e8f0">
                                {
                                    hourSchedules[toGetSchedulesDate] && hourSchedules[toGetSchedulesDate].map((schedule) => {
                                        const scheduleColor = schedule.status ? 'green' : (date.toDateString() == new Date().toDateString() ? 'orange' : date < new Date() ? 'red' : 'blue');

                                        const bgIntensity = profile && profile.id === schedule.user.id ? '400' : '100';
                                        const textColor = profile && profile.id === schedule.user.id ? 'white' : `${scheduleColor}.500`;

                                        return profile && profile.id === schedule.user.id ? (
                                            <Popover>
                                                <PopoverTrigger>
                                                    <Stack border="1px solid" cursor={profile && profile.id === schedule.user.id ? 'pointer' : 'alias'} key={`${schedule.city}-${schedule.user.name}-${day}-${hour}`} spacing="0" bg={`${scheduleColor}.${bgIntensity}`} color={textColor} p="1" textTransform="capitalize" borderRadius="4px" fontSize="11px" mb="3px">
                                                            ...
                                                            <Stack spacing="4">
                                                                {
                                                                    !schedule.status && (
                                                                        <>
                                                                            <OutlineButton onClick={() => handleCompleteSchedule({id: schedule.id, lead: schedule.lead && schedule.lead.id})} w="130px" h="28px" colorScheme="green" borderColor="green.400" color="green.400">Concluir</OutlineButton>
                                                                            <EditButton onClick={() => handleEditSchedule({id: schedule.id, city: schedule.city, datetime: schedule.datetime, lead: schedule.lead ? schedule.lead.id : 0})} w="130px"/>
                                                                        </>
                                                                    )
                                                                }
                                                                
                                                                <RemoveButton onClick={() => handleRemoveSchedule({id: schedule.id})} w="130px"/>
                                                            </Stack>
                                                        </PopoverBody>
                                                    </PopoverContent>
                                                </Portal>
