...
export default function Roles() {
  ...
  const [isConfirmRoleRemoveModalOpen, setIsConfirmRoleRemoveModalOpen] =
    useState(false)
  ...
  function handleChangeRole(event: any) {
    const selectedRoleData = roles.data.filter(
      (role: Role) => role.id === parseInt(event?.target.value)
    )[0]
  ...
  const handleSavePermissions = async (data: any) => {
    ...
    const filteredPermissions = Object.values(data).filter(
      (permissionKey, permissionIndex, permissionArray) => {
        return (
          parseInt(Object.keys(data)[permissionIndex].split('-')[0]) ===
            selectedRoleId && permissionKey !== false
        )
      }
    )

    //transforma o Array de permissões em um objeto do tipo SyncPermissions
    const syncedPermissions = filteredPermissions.reduce(
      (
  ...
  useEffect(() => {
    if (!roles.isLoading && !roles.error && selectedRoleId) {
      ...
      if (selectedRoleData) {
        setEditRoleData(
          roles.data.filter((role: Role) => role.id === selectedRoleId)[0]
        )
      }
    }
  ...
  return (
    <MainBoard sidebar="configs">
      <NewRoleModal
        afterCreate={roles.refetch}
        isOpen={isNewRoleModalOpen}
        onRequestClose={CloseNewRoleModal}
      />
      <EditRoleModal
        afterEdit={roles.refetch}
        toEditRoleData={editRoleData}
        isOpen={isEditRoleModalOpen}
        onRequestClose={CloseEditRoleModal}
      />
      <ConfirmRoleRemoveModal
        afterRemove={roles.refetch}
        toRemoveRoleId={selectedRoleId}
        isOpen={isConfirmRoleRemoveModalOpen}
        onRequestClose={CloseConfirmRoleRemoveModal}
      />
            ...
            <Stack spacing="10" direction="column">
              {permissions &&
                permissions.map(permission => {
                  ...
                  return (
                    <FormControl
                      key={`${selectedRoleId}-${permission.id}`}
                      pos="relative"
                    >
                      <Checkbox
                        {...register(`${selectedRoleId}-${permission.id}`)}
