...
export function BillsSummary({
  ...
  return (
    ...
    >
      ...
      {!bills.isLoading &&
        !bills.error &&
        Object.keys(bills.data?.data).map((company: string) => {
          const totalDayAmount = bills.data?.data[company].reduce(
            ...
            >
              ...
              >
                ...
                <Flex
                  alignItems="center"
                  float="right"
                  color={totalDayAmount > 0 ? 'green.400' : 'red.400'}
                >
                  {/* {totalDayAmount > 0
                  ...
                  >
                    <Stack>
                      <Flex
                        fontWeight="500"
                        alignItems="center"
                        opacity={bills.status ? 0.5 : 1}
                      >
                        <EllipseIcon
                          stroke="none"
                          fill={bills.category?.color}
                        />
                        <Text ml="2" color={bills.category?.color}>
                          {bills.title}
                        </Text>
                      </Flex>
