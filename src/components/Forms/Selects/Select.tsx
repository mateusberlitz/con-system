...
export function Select({ name, children, variant, selectRef, value = "", selected, error, register, onChange, ...rest } : FormSelectProps){
    ...
    return(
        <FormControl pos="relative" isInvalid={!!error}>
            <ChakraSelect {...getRegister()} h="45px" name={name} fontSize="sm" focusBorderColor="purple.600" borderColor={variant === 'outline' ? "gray.500" : "transparent"} bgColor={variant === 'outline' ? "gray.100" : "gray.400"} variant="filled" _hover={ {bgColor: 'gray.500'} } size="lg" borderRadius="full" color="gray.600">
                {children}
            </ChakraSelect>
